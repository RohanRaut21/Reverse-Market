  ...
  return (
    <div className="min-h-screen bg-darkBg text-white grid grid-cols-1 lg:grid-cols-12 font-sans antialiased overflow-x-hidden">
      ...
      <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-12 bg-[#0b0d19] border-r border-darkBg-border/40 relative overflow-hidden">
        ...
        {/* Brand Logo Header */}
        <div className="flex items-center gap-2 cursor-pointer relative z-10" onClick={onBackToLanding}>
        ...
        <div className="my-auto space-y-10 relative z-10">
          <div className="space-y-4">
            ...
            <p className="text-slate-400 text-sm max-w-sm leading-relaxed">
              Post what you need and get the best offers from verified sellers. Save time, save money.
            </p>
          </div>
          ...
          <div className="relative w-full h-[320px] flex items-center justify-center mt-6">
            ...
            <div className="absolute top-[20%] -left-6 bg-[#121426]/95 border border-darkBg-border/80 backdrop-blur-md rounded-2xl p-3 shadow-2xl flex flex-col w-36 transition-all hover:scale-105">
              <span className="text-[10px] text-slate-500 font-semibold">Offer Received</span>
              <span className="text-sm font-extrabold text-white mt-0.5">₹12,500</span>
              <div className="flex justify-between items-center mt-1.5">
                <span className="text-[10px] text-slate-400 truncate w-20">Global Suppliers</span>
                <span className="text-[9px] bg-brand-blue/10 text-brand-blue px-1.5 py-0.5 rounded font-bold">★ 4.6</span>
              </div>
            </div>
            ...
            <div className="absolute -top-6 right-2 bg-[#121426]/95 border border-brand/35 backdrop-blur-md rounded-2xl p-3 shadow-2xl flex flex-col w-36 transition-all hover:scale-105">
              <span className="text-[10px] text-yellow-400 font-bold flex items-center gap-0.5">🏆 Best Offer</span>
              <span className="text-sm font-extrabold text-white mt-0.5">₹10,800</span>
              <div className="flex justify-between items-center mt-1.5">
                <span className="text-[10px] text-slate-400 truncate w-20">Shree Traders</span>
                <span className="text-[9px] bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-bold">★ 4.9</span>
              </div>
            </div>
            ...
            <div className="absolute bottom-[10%] -right-4 bg-[#121426]/95 border border-darkBg-border/80 backdrop-blur-md rounded-2xl p-3 shadow-2xl flex flex-col w-36 transition-all hover:scale-105">
              <span className="text-[10px] text-slate-500 font-semibold">Offer Received</span>
              <span className="text-sm font-extrabold text-white mt-0.5">₹13,900</span>
              <div className="flex justify-between items-center mt-1.5">
                <span className="text-[10px] text-slate-400 truncate w-20">Rahul Enterprises</span>
                <span className="text-[9px] bg-brand/10 text-brand px-1.5 py-0.5 rounded font-bold">★ 4.8</span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-darkBg-border/40 pt-6 mt-auto">
          <div>
            <h4 className="text-[11px] font-bold text-white uppercase tracking-wide">Verified Sellers</h4>
            <p className="text-[10px] text-slate-500 leading-snug mt-1">All sellers are fully verified for safety and trust.</p>
          </div>
          <div>
            <h4 className="text-[11px] font-bold text-white uppercase tracking-wide">Best Prices</h4>
            <p className="text-[10px] text-slate-500 leading-snug mt-1">Compare multiple offers and choose the best fit.</p>
          </div>
          <div>
            <h4 className="text-[11px] font-bold text-white uppercase tracking-wide">Secure & Safe</h4>
            <p className="text-[10px] text-slate-500 leading-snug mt-1">Your data, transactions, and payouts are protected.</p>
          </div>
        </div>
      </div>
      ...
      <div className="col-span-1 lg:col-span-7 flex flex-col justify-center items-center w-full min-h-screen py-12 px-6 relative">
        ...
        <div className="w-full max-w-lg bg-[#121426] border border-darkBg-border/80 rounded-3xl p-8 shadow-2xl relative flex flex-col">
          ...
          <div className="mb-6 p-3 bg-darkBg border border-darkBg-border/70 rounded-2xl flex items-center justify-between text-xs">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => prefillUser('Buyer')}
                className="px-2.5 py-1 bg-brand/10 hover:bg-brand/20 text-brand font-bold rounded-lg transition-all"
              >
                Rohan (Buyer)
              </button>
              <button
                type="button"
                onClick={() => prefillUser('Seller')}
                className="px-2.5 py-1 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 font-bold rounded-lg transition-all"
              >
                Arjun (Seller)
              </button>
            </div>
          ...
          <form onSubmit={isLogin ? handleLogin : handleRegister} className="space-y-4">
            ...
            {!isLogin && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                >
                  <User className={`w-5 h-5 ${regRole === 'Buyer' ? 'text-brand' : 'text-slate-400'}`} />
                  <div>
                    <span className="text-xs font-bold text-white block">I'm a Buyer</span>
                    <span className="text-[10px] text-slate-500 block leading-tight mt-0.5">Post requests and get offers</span>
                  </div>
                </button>
                ...
                >
                  <Briefcase className={`w-5 h-5 ${regRole === 'Seller' ? 'text-brand' : 'text-slate-400'}`} />
                  <div>
                    <span className="text-xs font-bold text-white block">I'm a Seller</span>
                    <span className="text-[10px] text-slate-500 block leading-tight mt-0.5">Sell and grow your business</span>
                  </div>
                </button>
              </div>
            )}
            ...
            {!isLogin && (
              <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-[10px] text-slate-500 font-semibold pt-1">
                <span className={`flex items-center gap-1.5 ${hasMinLength ? 'text-emerald-400' : 'text-slate-500'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${hasMinLength ? 'bg-emerald-400' : 'bg-slate-500'}`}></span>
                  At least 8 characters
                </span>
                <span className={`flex items-center gap-1.5 ${hasNumber ? 'text-emerald-400' : 'text-slate-500'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${hasNumber ? 'bg-emerald-400' : 'bg-slate-500'}`}></span>
                  One number
                </span>
                <span className={`flex items-center gap-1.5 ${hasSpecial ? 'text-emerald-400' : 'text-slate-500'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${hasSpecial ? 'bg-emerald-400' : 'bg-slate-500'}`}></span>
            ...
            {isLogin && (
              <div className="text-right">
                <a href="#" className="text-xs text-brand hover:underline font-semibold">Forgot Password?</a>
              </div>
            )}
          ...
          >
            <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v3.9h6.69c-.29 1.5-.1.8-1.5 2.1l2.3 2.1 2.3 2.1c1.5-1.5 2.4-3.5 2.4-6.1z"/>
              <path fill="#34A853" d="M12 24c3.24 0 5.97-1.08 7.96-2.91l-3.8-2.94c-1.1.74-2.5 1.18-4.16 1.18-3.2 0-5.9-2.16-6.87-5.06H1.18v3.08C3.18 21.3 7.28 24 12 24z"/>
              <path fill="#FBBC05" d="M5.13 14.27c-.25-.74-.39-1.54-.39-2.37s.14-1.63.39-2.37V6.45H1.18C.43 7.94 0 9.61 0 11.9s.43 3.96 1.18 5.45l3.95-3.08z"/>
              <path fill="#EA4335" d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.43-3.43C17.96 1.19 15.23 0 12 0 7.28 0 3.18 2.7 1.18 6.45l3.95 3.08c.97-2.9 3.67-5.06 6.87-5.06z"/>
            </svg>
          ...
          <div className="grid grid-cols-4 gap-2 pt-6 mt-6 border-t border-darkBg-border/40 text-center text-[9px] text-slate-500 leading-tight">
            <div>
              <strong className="text-slate-400 block font-bold mb-0.5">100% Free</strong>
              <span>No hidden charges</span>
            </div>
            <div>
              <strong className="text-slate-400 block font-bold mb-0.5">Trusted Platform</strong>
              <span>Verified & secure</span>
            </div>
            <div>
              <strong className="text-slate-400 block font-bold mb-0.5">24/7 Support</strong>
              <span>We're here to help</span>
            </div>
            <div>
              <strong className="text-slate-400 block font-bold mb-0.5">Quick & Easy</strong>
