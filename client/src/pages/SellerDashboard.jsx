...
const SellerDashboard = ({ user, onPlaceBidClick, onViewRequestClick, activeTab, setActiveTab }) => {
  ...
  const fetchAvailableRequests = async () => {
    try {
      ...
      const categoryParam = currentCategory === 'All Requests' ? 'All' : currentCategory;
      const res = await fetch(`/api/requests?status=Active&category=${categoryParam}`);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-darkBg-card border border-darkBg-border p-6 rounded-2xl flex items-center justify-between">
          <div className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block">Total Bids</span>
            <span className="text-3xl font-extrabold text-white block">{stats.totalBids}</span>
            <span className="text-[11px] text-emerald-400 font-semibold block">• 6 new this week</span>
          </div>
        ...
        <div className="bg-darkBg-card border border-darkBg-border p-6 rounded-2xl flex items-center justify-between">
          <div className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block">Bids in Progress</span>
            <span className="text-3xl font-extrabold text-white block">{stats.inProgress}</span>
            <span className="text-[11px] text-brand-blue font-semibold block">• 2 awaiting response</span>
          </div>
        ...
        <div className="bg-darkBg-card border border-darkBg-border p-6 rounded-2xl flex items-center justify-between">
          <div className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block">Orders Won</span>
            <span className="text-3xl font-extrabold text-white block">{stats.ordersWon}</span>
        ...
        <div className="bg-darkBg-card border border-darkBg-border p-6 rounded-2xl flex items-center justify-between">
          <div className="space-y-2">
            ...
            <span className="text-3xl font-extrabold text-white block">{stats.completedOrders}</span>
            <span className="text-[11px] text-emerald-400 font-semibold block">• 12 this month</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 bg-darkBg-card border border-darkBg-border rounded-3xl p-6 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h3 className="text-lg font-bold text-white">Latest Buyer Requests</h3>
            ...
            <div className="flex bg-darkBg border border-darkBg-border p-0.5 rounded-xl text-xs font-semibold overflow-x-auto max-w-full">
              {['All Requests', 'Electronics', 'Home & Kitchen', 'Fashion', 'Services', 'Vehicles', 'Others'].map((tab) => (
                <button
                  key={tab}
          ...
          <div className="divide-y divide-darkBg-border">
            {loading ? (
              <div className="py-12 text-center text-slate-500">Loading requests feed...</div>
            ) : requests.length === 0 ? (
              <div className="py-12 text-center text-slate-500">No active buyer requests found in this category.</div>
            ) : (
              requests.map((req) => (
                <div 
                  key={req._id}
                ...
                >
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-bold text-white hover:text-brand transition-colors cursor-pointer" onClick={() => onViewRequestClick(req)}>
                          {req.title}
                        </h4>
                        {req.featured && (
                      ...
                      <div className="flex flex-wrap gap-2 pt-1 text-[10px] text-slate-400">
                        ...
                        {req.tags && req.tags.map((tag, i) => (
                          <span key={i} className="bg-darkBg border border-darkBg-border px-2 py-0.5 rounded-md">
                            {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-8 border-t border-darkBg-border/40 md:border-t-0 pt-3 md:pt-0">
                    <div className="text-left md:text-right">
                      <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Budget</span>
                      <span className="text-sm font-extrabold text-white block">
                        ₹{req.budget.toLocaleString()}
                      </span>
                      <span className="text-[10px] text-slate-400 block mt-0.5">
                        Deadline: {getRemainingDays(req.deadline)} ({req.bidCount || 0} bids placed)
                      </span>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-darkBg-card border border-darkBg-border rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div className="space-y-4 text-xs">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-slate-300">Your bid on <strong>"Wireless Headphones"</strong> was viewed by the buyer</p>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-slate-300">You were outbid on <strong>"Gaming Laptop"</strong> by TechGear Solutions</p>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-slate-300">Your bid on <strong>"Bulk T-Shirts"</strong> was shortlisted</p>
          ...
          <div className="bg-darkBg-card border border-darkBg-border rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div className="divide-y divide-darkBg-border text-xs">
              {[
                { label: 'All Bids', count: bidsSummary.all, color: 'bg-brand/20 text-brand' },
                { label: 'In Progress', count: bidsSummary.inProgress, color: 'bg-blue-500/20 text-blue-400' },
                { label: 'Shortlisted', count: bidsSummary.shortlisted, color: 'bg-yellow-500/20 text-yellow-400' },
                { label: 'Outbid', count: bidsSummary.outbid, color: 'bg-red-500/20 text-red-400' },
                { label: 'Withdrawn', count: bidsSummary.withdrawn, color: 'bg-slate-500/20 text-slate-400' },
                { label: 'Accepted', count: bidsSummary.accepted, color: 'bg-emerald-500/20 text-emerald-400' }
              ].map((item, idx) => (
                <div key={idx} className="flex justify-between items-center py-2.5 first:pt-0 last:pb-0 cursor-pointer group hover:bg-darkBg-hover/30 px-1.5 -mx-1.5 rounded-lg transition-all">
                  <div className="flex items-center gap-2">
          ...
          <div className="bg-darkBg-card border border-darkBg-border rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <span className="text-[10px] text-slate-400 font-semibold block leading-tight">Browse Requests</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] text-slate-400 font-semibold block leading-tight">My Bids</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] text-slate-400 font-semibold block leading-tight">Messages</span>
              </button>
            </div>
          </div>
        </div>
